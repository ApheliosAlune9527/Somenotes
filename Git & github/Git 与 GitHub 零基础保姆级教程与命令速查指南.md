...
[![188](https://img.youtube.com/vi/Ez8F0nW6S-w/maxresdefault.jpg)](https://www.youtube.com/watch?v=Ez8F0nW6S-w&t=4299s)
## 目录
...
- **命令行终端**：
    ...
    - **Windows 用户**：必须下载并安装 [**Git Bash** ](https://git-scm.com/install/windows)（安装时一路点击 Next 即可，建议勾选 "On the Desktop" 图标，并在命名默认分支时可选择将其设为 `main` ）。
...
- **什么是 PR？**：在团队协作中，你不能直接把自己的分支合并到生产环境的 `main` 分支。你需要提交一个“拉取请求”（Pull Request），告诉团队成员：“我写好了一个新功能，请求将我的 `feature-login` 分支合并到 `main`。”
	...
	- 方法2 : 网页 点击 _绿色按钮 Compare & pull request_ 在弹出的页面里边github的Copilot会自动判断是否有冲突
		![](附件/网页PR.png)
...
- 两个分支（比如 `main` 和 `feature-login）` ) 同时修改了**同一个文件的同一行代码**。
