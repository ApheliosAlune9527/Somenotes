...
#     例如：(4,3) 与 (3,) -> (3,) 变为 (1,3)，绝不会补在右边变成 (3,1)！
#   - 第二步：逐维检查，满足任一条件即可广播：
#     ① 该维度数字完全相等
#     ② 其中一方该维度为 1
#   - 只要有一维不符合 -> 报错 ValueError (Shape Mismatch)
#
# 2. 内存本质：【步长 (Strides) 设为 0】
#   - 广播绝对没有在物理内存里复制、扩容小数组
#   - 它是将需要拉伸的维度步长设为 0，让 CPU 循环时指针"原地打转"，反复读取同一份内存
#
# 3. 避坑指南：
#   - 0 维标量  `5`        -> 形状 `()`   : 适配任何维度
#   - 1 维行向量 `[1,2,3]`  -> 形状 `(3,)`  : 广播时无条件补维成 `(1, 3)`
#   - 2 维列向量            -> 形状 `(3, 1)`: 只能横向拉伸
#   - ⚠️ (3,) + (3, 1) 会双向广播，产生 (3, 3) 矩阵！
#     若想对应元素计算，请用 .reshape(3, 1) 或 np.newaxis 强锁 2D 维度
#
# ==================== 演练场（从右往左看） ====================
# A: (4, 3)  +  B: (3,)   -> B 补为 (1, 3) -> 末尾 3==3, 倒数 1变4 -> 成功 -> (4, 3)
# A: (3, 3)  +  B: (3, 1) -> 末尾 1变3, 倒数 3==3                     -> 成功 -> (3, 3)
# A: (4, 3)  +  B: (4,)   -> B 补为 (1, 4) -> 末尾 3与4 不匹配         -> 报错！
#
# A: (2, 3, 4) + B: (3, 1) -> 最后维 4与1（有1，通过）；倒数第二维 3==3（通过）；
#                              最前面 2与1（有1，通过）-> 成功 -> (2, 3, 4)
#
# ==================== 双向广播实例：(3,) + (3, 1) ====================
...
#   7  8  9
#
# 注意：numpy 没有真的复制数据，步长设为 0 实现虚拟拉伸，效率很高
```
