...
> - **切片规则**：**左闭右开** `[start:stop)`，包含 start，不包含 stop
> - **二维索引**：`arr[行, 列]`，用逗号分隔行和列

> [!example]- 1. 一维数组：基本索引与切片
> ```python
> import numpy as np
...
> print(arr[-1])     # 100  最后一个元素
> print(arr[-2])     # 90   倒数第二个
>
> # ========== 切片 arr[start:stop:step] ==========
> print(arr[:])      # [10 20 ... 100]  全部元素
> print(arr[2:6])    # [30 40 50 60]    索引2到5（不含6）
> print(arr[::2])    # [10 30 50 70 90] 每隔一个取一个
> print(arr[::-1])   # [100 90 ... 10]  反转数组
> print(arr[1:8:2])  # [20 40 60 80]    从索引1到7，步长2
>
> # ========== slice 对象（等价写法） ==========
> print(arr[slice(1, 5)])      # 等价于 arr[1:5]
> print(arr[slice(None, None, -1)])  # 等价于 arr[::-1]
> ```

> [!example]- 2. 二维数组：索引与切片
> ```python
> import numpy as np
...
>
> # ========== 取整行 / 整列 ==========
> print(arr[0, :])    # [1 2 3 4]     第1行全部
> print(arr[1, :])    # [5 6 7 8]     第2行全部
> print(arr[:, 0])    # [1 5 9]       第1列全部
> print(arr[:, -1])   # [4 8 12]      最后一列全部
>
> # ========== 子矩阵切片 ==========
> print(arr[0:2, 1:3])   # [[2 3]      前2行，第2~3列
>                         #  [6 7]]
> print(arr[::2, :])     # [[ 1  2  3  4]   隔行取，第1和第3行
>                         #  [ 9 10 11 12]]
> print(arr[:, ::2])     # [[ 1  3]     隔列取，第1和第3列
>                         #  [ 5  7]
>                         #  [ 9 11]]
>
> # ========== 取单行（降维） vs 取单行切片（保持2D） ==========
> print(arr[1, :])       # [5 6 7 8]      → 一维数组 (4,)
> print(arr[1:2, :])     # [[5 6 7 8]]    → 二维数组 (1, 4)
> ```

> [!tip]- 3. 切片是视图，不是副本！
> **关键区别**：NumPy 的切片返回的是原数组的 **视图（view）**，修改切片会直接影响原数组！
>
> ```python
> import numpy as np
> arr = np.array([1, 2, 3, 4, 5])
...
> |---|---|---|
> | `arr[1:4]` | 视图（view） | ✅ 影响 |
> | `arr[1:4].copy()` | 副本（copy） | ❌ 不影响 |
> | `np.array(arr[1:4])` | 副本（copy） | ❌ 不影响 |
> | 布尔索引 `arr[arr>5]` | **副本** | ❌ 不影响 |
> | 花式索引 `arr[[0,2]]` | **副本** | ❌ 不影响 |

> [!example]- 4. 布尔索引（条件筛选）
> 用布尔数组当"筛子"：True 的位置留下，False 的位置丢掉。
>
> ```python
> import numpy as np
...
> print(arr[mask])      # [7 9 8]  只留下 > 5 的元素
>
> # 可以一行写完
> print(arr[arr > 5])   # [7 9 8]
> print(arr[arr % 2 == 0])   # [4 8 2]  只留下偶数
>
> # ========== 多条件组合 ==========
> # ⚠️ 每个条件必须加括号，用 &（且）和 |（或）连接
> print(arr[(arr > 3) & (arr < 9)])   # [7 4 8]   大于3且小于9
> print(arr[(arr < 3) | (arr > 8)])   # [1 9 2]   小于3或大于8
> print(arr[~(arr > 5)])              # [3 1 4 2] 取反，即 <= 5
>
> # ========== 二维布尔索引 ==========
> A = np.array([[12, 3, 15],
>               [ 2, 11,  9]])
>
> print(A > 10)         # [[ True False  True]
>                       #  [False  True False]]
> print(A[A > 10])      # [12 15 11]  结果降为一维！不再保留行列结构
>
> # 如果想保留行列结构，用 np.where()
> result = np.where(A > 10, A, 0)  # 满足条件保留原值，否则填0
> print(result)         # [[12  0 15]
>                       #  [ 0 11  0]]
>
> # ========== 统计满足条件的个数 ==========
> print(np.sum(arr > 5))           # 3   True算1，False算0
> print(np.count_nonzero(arr > 5)) # 3   等价写法
> ```

> [!example]- 5. 花式索引（整数数组索引）
> 用整数数组指定要取的索引位置，可以任意顺序取元素。
>
> ```python
> import numpy as np
...
>
> # ========== 一维花式索引 ==========
> print(arr[[0, 2, 5]])      # [10 30 60]  取索引 0、2、5 的元素
> print(arr[[4, 1, 7, 1]])   # [50 20 80 20]  可以重复，可以乱序
>
> # ========== 二维花式索引 ==========
> B = np.array([[1, 2, 3],
>               [4, 5, 6],
...
>
> # 取指定行
> print(B[[0, 2], :])        # [[1 2 3]    取第1行和第3行
>                            #  [7 8 9]]
>
> # 取指定列
> print(B[:, [0, 2]])        # [[1 3]      取第1列和第3列
>                            #  [4 6]
>                            #  [7 9]]
>
> # 取特定位置的元素（行和列一一对应）
> rows = [0, 1, 2]
> cols = [1, 2, 0]
> print(B[rows, cols])       # [2 6 7]  即 B[0,1], B[1,2], B[2,0]
>
> # 花式索引返回的是 副本，不是视图
> sub = B[[0, 2]]
> sub[0, 0] = 999
> print(B[0, 0])            # 1  原数组不受影响
> ```

> [!warning]- 常见踩坑点
> 1. **二维布尔索引会降维**：`A[A > 10]` 返回一维数组，想保留形状用 `np.where()`
> 2. **切片是视图**：`arr[1:3][0] = 99` 会改原数组，想要副本用 `.copy()`
> 3. **多条件必须加括号**：`arr[arr > 3 & arr < 9]` ❌ → `arr[(arr > 3) & (arr < 9)]` ✅
> 4. **逗号不能忘**：二维索引是 `arr[行, 列]`，不是 `arr[行][列]`（后者多一层中间索引）
