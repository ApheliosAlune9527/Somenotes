...
1.  同质性 : ndarray中的元素必须是**同一种数据类型**,如果混合了不同的数据类型,Numpy会自动将他们转换为一种兼容的类型。
<br>

2.  大小固定 : 一旦创建了 ndarray , 它的大小是固定的,不能动态调整。如果需要改变大小 , 需要创新一个新的 ndarray。
...
<br>
2. 数组的维度 :   ndim 用法 : `arr.ndim` 返回一个整数 , bi
```python
"""Ndarry的属性
