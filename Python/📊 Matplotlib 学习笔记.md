...
- [[#二、折线图 (Line Plot) 基础与 NumPy 整合|二、折线图基础]]
- [[#三、折线与数据标记 (Markers) 的个性化定制|三、Markers 定制]]
- [[#四、高级标签、标题与刻度定制|四、标签与刻度]]
- [[#五、参考网格线 (Grid Lines) 的配置|五、网格线]]
- [[#六、条形图 (Bar Chart) 与水平条形图|六、条形图]]
- [[#七、饼图 (Pie Chart) 深度定制|七、饼图]]
- [[#八、散点图 (Scatter Plot) 与相关性分析|八、散点图]]
- [[#九、直方图 (Histogram) 与分布可视化|九、直方图]]
- [[#十、画布与多子图网格 (Figure & Subplots)|十、子图]]
- [[#十一、Pandas 与 Matplotlib 综合实战项目 (宝可梦数据分析)|十一、Pandas 实战]]

---

## 一、Matplotlib 简介与安装配置
...
## 八、散点图 (Scatter Plot) 与相关性分析

1. **作用**：探究和展示两个连续变量之间的关联规律（如：正相关、负相关、不相关）。
	<br>
2. **透明度 (Alpha)**：通过控制 `alpha` 的取值区间 `[0.0, 1.0]`，可以极好地解决因数据点过于密集导致的”数据点重叠覆盖”问题。
	<br>
...
## 九、直方图 (Histogram) 与分布可视化

**直方图 vs 条形图：**
- **条形图**：展示定性分类数据（如品牌、城市）。
- **直方图**：展示定量连续分布数据（如年龄分布、考试得分区间），它通过自动或者指定的 **区间桶 (Bins)** 将数值分组并统计其频数。

1. **高斯/正常分布生成 (`np.random.normal`)**：`loc` 均值（正态分布中心），`scale` 标准差（越小分布越陡峭，越大越分散）。
	<br>
2. **边界约束 (`np.clip`)**：用于对异常数据进行约束。在模拟考试分数时，通过 `np.clip` 可以将低于 `0` 分或大于 `100` 分的极端值规整到合理范围内。
	<br>
3. **`plt.hist()` 的返回值**：
	...
	<br>
4. **柱子数值标注技巧**：利用 `plt.hist()` 的返回值，配合 `plt.text()` 和 `zip()` 循环，可以在每个柱子上方标注频数。
...
## 十、画布与多子图网格 (Figure & Subplots)

1. **画布 (Figure)**：底层的全局图像窗口容器（也就是空白纸张）。
	<br>
2. **轴域/子图 (Axes/Subplot)**：Figure 画布内部承载单个独立图表坐标系的最小单元。
	<br>
3. **多维网格解包 (Tuple Unpacking)**：`plt.subplots(nrows, ncols)` 会返回 Figure 对象和一整组 Axes 子图对象。通过 2D 数组索引定位（如 `axes[row, col]`）便能分别操作每一张子图。
	<br>
4. **排版紧凑化布局 (`plt.tight_layout`)**：Matplotlib 的默认排版容易导致各子图的 X 轴标签与上一行子图的 Title 重合。执行 `tight_layout()` 可以自适应紧凑调整，彻底消除重叠现象。
...
## 十一、Pandas 与 Matplotlib 综合实战项目 (宝可梦数据分析)
...
| 模块 | 重点内容 |
|------|----------|
| 简介与安装 | matplotlib.pyplot 核心模块、基本工作流 |
| 折线图 | plot()、NumPy 数组、数据对称性约束 |
| Markers 定制 | marker、linestyle、color、字典解包 `**` |
| 标签与刻度 | title、xlabel/ylabel、xticks、tick_params |
| 网格线 | grid()、axis 方向控制、linestyle/color |
| 条形图 | bar()、barh()、离散类别数据 |
| 饼图 | pie()、autopct、explode、shadow、startangle |
| 散点图 | scatter()、alpha 透明度、label + legend |
| 直方图 | hist()、bins、np.clip 约束、plt.text() 标注 |
| 子图 | subplots()、axes[row, col]、tight_layout |
| Pandas 实战 | value_counts + barh 水平条形图 |

### 关键要点

1. **字典解包**：用 `dict()` 或 `{}` 统一样式，`**dict` 解包传入，减少重复代码
2. **hist() 返回值**：`counts, bins = plt.hist(...)` 可用于后续标注等操作
3. **tight_layout()**：多子图或长标签时必加，防止文字重叠
