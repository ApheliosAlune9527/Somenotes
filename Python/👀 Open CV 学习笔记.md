...
1.  **Canny 边缘检测**：它通过计算像素点左右、上下的"亮度差（灰度梯度）"来找边界。三维颜色向量是没法直接算导数（偏微分）的，必须降维成一维灰度。
2.  **人脸检测（Haar特征）**、**特征点匹配（ORB/SIFT）**：这些算法的设计初衷就是分析局部的明暗变化。

所以，像 `cv.Canny()` 这样的函数，你如果传一张彩色图进去，它在底层也会**强行先把它转成灰度图**再处理。

---

### ⚠️ 控制工程师的清醒：什么时候"绝对不能"灰度化？

既然灰度化这么好，为什么识别红色物体不能用它？

*   **因为你的目标特征就是"颜色"本身！**
*   如果你把彩色图转成了灰度图，红色、绿色、蓝色在灰度图里可能都会变成一模一样的"某种灰色"（因为它们的亮度可能恰好相同）。你将彻底失去区分它们的能力。
